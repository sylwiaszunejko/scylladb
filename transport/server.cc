...
make_result(int16_t stream, messages::result_message& msg, const tracing::trace_state_ptr& tr_state,
    ...
    if (!msg.custom_payload().empty()) {
        response->set_frame_flag(cql_frame_flags::custom_payload);
        response->write_string_bytes_map(msg.custom_payload());
    }
...
void cql_server::response::write_string_bytes_map(std::map<sstring, bytes> map)
{
